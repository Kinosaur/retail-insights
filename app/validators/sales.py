...
def validate_sales(df: pd.DataFrame) -> tuple[list[dict], list[dict]]:
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            continue
        if sale_date > today:
            errors.append({"row": row_num, "sku_id": sku_id, "reason": "future date not allowed"})
            continue
        ...
        try:
            ...
            errors.append({"row": row_num, "sku_id": sku_id, "reason": "quantity must be an integer"})
            continue
        if quantity == 0:
            errors.append({"row": row_num, "sku_id": sku_id, "reason": "quantity cannot be zero"})
            continue
        ...
        try:
            ...
            errors.append({"row": row_num, "sku_id": sku_id, "reason": "unit_price must be a number"})
            continue
        if unit_price <= 0:
            errors.append({"row": row_num, "sku_id": sku_id, "reason": "unit_price must be > 0"})
            continue

        # 5. duplicate — same sku_id + date + qty + price → silently skip
        dedup_key = (sku_id, sale_date, quantity, unit_price)
